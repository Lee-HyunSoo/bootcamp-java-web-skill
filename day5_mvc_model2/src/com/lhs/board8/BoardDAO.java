...
public class BoardDAO {
	...
	public List<ArticleVO> selectAllArticles(Map pagingMap){
		...
		try{
		   conn = dataSource.getConnection();
		   String query ="SELECT * FROM (select ROWNUM  as recNum, LVL, articleNO, parentNO, title, id, writeDate from "
		   				+ "(select LEVEL as LVL, articleNO, parentNO, title, id, writeDate from t_board START WITH parentNO=0"
